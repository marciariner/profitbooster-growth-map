// api/results.ts — HTML Results page served as an Edge Function
...
export default async function handler(req: Request) {
  try {
    ...
    // Pull values from querystring (from GHL redirect)
    const payload = {
    ...
    // Call your own API function on the server (no CORS)
    const apiUrl = new URL('/api/growth-map', url.origin);
    ...
    const list = Array.isArray(out.moves) ? out.moves.map((m: any) =>
      `<li><strong>${htmlescape(m.name)}</strong> — <em>What:</em> ${htmlescape(m.what)}. <em>Why:</em> ${htmlescape(m.why)}.</li>`
    ...
    return html(`
      <div class="wrap"><div class="card">
        ...
        <p><strong>Your profit position today:</strong> ${htmlescape(out.positionLabel || '')}${out.score ? ` (${out.score}/100)` : ''}</p>
        <ol>${list}</ol>
        <p><strong>Your next best action:</strong> ${htmlescape(out.nextAction || 'We’ll email your next step shortly.')}</p>
        <a href="https://profitbooster.biz/booking" class="cta">Book Your Growth Jumpstart</a>
      </div></div>
    `);
  } catch (e) {
    return html(`<div class="wrap"><div class="card"><div class="err">Unexpected error.</div></div></div>`, 500);
  }
}
...
function html(body: string, status = 200) {
  ...
  return new Response(base, { status, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
}
